...
import { SimpleChatHandler, SimpleChatConfig, SimpleChatCallbacks } from '@/lib/simple-chat-handler';
...
/**
 * Simplified chat hook that bypasses WebSocket complexity and uses direct HTTP calls
 * This replaces useWebSocketChat for the chat integration fix
 */
export function useSimpleChat(config: UseSimpleChatConfig = {}): UseSimpleChatReturn {
  ...
  useEffect(() => {
    const callbacks: SimpleChatCallbacks = {
      onMessage: (message: ChatMessage) => {
        console.log('useSimpleChat: Received message:', message);
  ...
  const sendQuestion = useCallback(async (question: string, conversationId?: string) => {
    ...
    try {
      ...
      console.log('useSimpleChat: Handler result:', result);
